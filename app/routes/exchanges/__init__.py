from flask import Blueprint, request, render_template, url_for, current_app, session, redirect, jsonify
import time
...
@bp.route("/exchanges")
def exchanges():
    oob_id = request.args.get('_oobid')
    exchange = await_(askar.fetch('exchange', oob_id))
    if not exchange:
        return {}, 404
    return exchange
